...
export default {
  ...
  mounted() {
    ...
    this.normalPosition = this.$refs.header.offsetTop; //距离顶部高度
  },
  ...
  methods: {
    scrollEvent() {
      ...
      this.timer = setTimeout(() => {
        if (
          document.documentElement.scrollTop >= this.normalPosition - 44 &&
          !this.isHeaderTop
        ) {
          console.log("处于顶部");
        ...
        for (let index in parts) {
          if (document.documentElement.scrollTop > parts[index].offsetTop - 89) {
            part = index;
          }
        }
      ...
      //   let normalPosition = this.$refs.header.offsetTop;
      //距离顶部的高度，等于滑动高度加上最上面不动的页面顶部的值时，该元素已吸在第二层顶部
    },
    toPart(index) {
      console.log("点击导航");
      ...
      let topDistance = this.$refs[index].offsetTop - 88; //获取元素至顶部的距离，注意要减去两个header
      window.scrollTo(0, topDistance); //滑动到指定位置，x和y
    },
  },
};
</script>
